...
namespace HFYStorySorter.Services
{
    public class StorySorterService : BackgroundService
    {
        ...
        public StorySorterService(IServiceProvider serviceProvider, ILogger<StorySorterService> logger, ServiceStatus status, IHttpClientFactory httpClientFactory)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    _logger.LogInformation("StorySorterService will run next at {Time}", DateTime.Now.AddMinutes(5));
        ...
        private async Task SortUnprocessedPostsAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (var post in unprocessedPosts)
            {
                ...
                var (storyTitle, chapterNumber) = await GetStoryInfoWithLLM(post.Title);
        ...
        //a lot slower but much more precise. tested with Mistral 7b, maybe works with llama3.2 3b or even 1b as well?
        private async Task<(string StoryTitle, int ChapterNumber)> GetStoryInfoWithLLM(string title)
        {
            string? result = null;
            try
            {
                ...
                var response = await httpClient.PostAsync("http://localhost:8000/parse_title", content);

                response.EnsureSuccessStatusCode();

                result = await response.Content.ReadAsStringAsync();
                var doc = JsonDocument.Parse(result);
                string storyTitle = doc.RootElement.GetProperty("story_title").GetString();
                int chapterNumber = doc.RootElement.GetProperty("chapter_number").GetInt32();

                return (storyTitle, chapterNumber);
            }
            catch (HttpRequestException httpEx)
            {
                _logger.LogError(httpEx, "HTTP error when calling LLM service for post title: {Title}", title);
            }
            catch (Exception ex)
            {
                //chapters can be interludes, half-chapters or subchapters (xyz chapter: 3.5) or multiple chapters(I Cast Gun, Chapter's 4 & 5), those cases are not handled by the db schema right now and the llm gets confused with them.
                _logger.LogError(ex, "Unexpected error when calling LLM service for post title: {Title}, Raw response: {RawResponse}", title, result ?? "<no response>");
            }
            return (string.Empty, 1); //will skip this post, handled in SortUnprocessedPostsAsync check for empty title
        }
    }
}
